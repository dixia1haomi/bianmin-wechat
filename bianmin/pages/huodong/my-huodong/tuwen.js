...
Page({
  ...
  onLoad: function (op) {
    ...
    this.setData({
      Res: prePage.data.huodongRes.withhuodong_img, huodong_id: prePage.data.huodongRes.id
    })
  },
  ...
  content_queding_() {
    ...
    cos.update_img_cos(cospath, img, (back) => {
      // 上传成功
      let params = {
        huodong_id: this.data.huodong_id,
        url: back.data.source_url,
        text: this.data.content.text.value
      }
      // 2.API新增商家IMG
      api.createHuodongImg({ params: params }, res => {
        console.log('新增活动IMG-OK', res)
        // 更新Res
        let Res = this.data.Res
        Res.push(res.data)
        this.setData({ Res: Res, 'content.img': [], 'content.state': false, 'content.text.value': '', 'content.text.cursor': 0 })
      })
    }, (ok) => {
      console.log('ok', ok)
    })
