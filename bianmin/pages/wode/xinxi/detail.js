...
Page({
  ...
  // 顶置
  dingzhi_() {
    console.log('dingzhi_')
  },

  // 删除
  shanchu_(e) {
    console.log('shanchu_')
    if (app.data.LoginState) {
      wx.showModal({
        title: '删除这条信息？',
        content: '将会删除信息相关的所有数据。',
        success: (res) => {
          if (res.confirm) {
            api.deleteMyFabu({
  ...
  yaoqing_haibao_() {
    ...
    api.xinxiDingzhierweima({
      ...
      if (back.msg == 'ok') {
        // 显示图片
        this.setData({
          haibaoImg: 'https://bianmin.qujingdaishuyanxuan.org' + back.data
        })
        // 提示
        wx.showModal({
          title: '制作完成',
  ...
  huifu_(e) {
    ...
    this.setData({
      ...
      huifu_detail: e.detail
    })
  },

  // 回复窗确定
  huifuchuang_queding_(e) {
    console.log('回复窗确定', e.detail)
    // 新增回复
    api.createBianminHuifu({
      ...
      neirong: e.detail.input, // 回复内容，写入回复表
      form_id: e.detail.form_id, // 写入便民信息表，有留言再次提醒
